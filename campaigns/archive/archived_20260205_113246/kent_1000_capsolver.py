...
class Kent1000CapSolverCampaign:
    ...
    def __init__(self, target_jobs: int = 1000):
        ...
        print("="*70)
        print("🚀 KENT LE - 1000 JOB CAMPAIGN WITH CAPSOLVER")
        print("="*70)
        print()
        ...
        if self.captcha_manager.is_configured():
            print("✅ CapSolver configured - CAPTCHA solving ENABLED")
            print(f"   API Key: {self.captcha_manager.api_key[:8]}...")
        else:
            print("⚠️  CapSolver NOT configured")
    ...
    async def apply_to_job(self, job: JobPosting, job_num: int) -> Dict:
        ...
        print(f"\n{'='*70}")
        print(f"📋 Job {job_num}/{self.target_jobs}: {job.title} at {job.company}")
        ...
        try:
            ...
            page = session.page
            print(f"      ✅ Session: {session.mode.value}")
            ...
            if captcha_result.success:
                print(f"      ✅ CAPTCHA solved (${captcha_result.cost:.4f})")
                ...
                print(f"      ❌ CAPTCHA solve failed: {captcha_result.error}")
            ...
            if not cloudflare_cleared:
                print("      ⚠️  Cloudflare still present")
            ...
            if validation.get("success"):
                print("      ✅ REAL SUBMISSION - Success indicators found!")
                self.stats.real_submissions += 1
                result["success"] = True
            elif validation.get("is_cloudflare"):
                print("      ❌ Cloudflare blocked submission")
                self.stats.cloudflare_blocked += 1
                result["error"] = "Cloudflare blocked"
            else:
                print(f"      ❌ No success indicators - {validation.get('message', 'Unknown')}")
    ...
    async def run(self):
        ...
        if self.captcha_manager.is_configured():
            ...
            print(f"\n❌ CapSolver NOT configured - will get 0% success rate")
            print("   Set CAPSOLVER_API_KEY to continue")
            return
        
        confirm = "CAPSOLVER1000"  # Auto-confirmed for overnight run
        ...
        if len(jobs) < self.target_jobs:
            print(f"⚠️  Only found {len(jobs)} jobs, adjusting target")
    ...
    def _print_final_stats(self):
        ...
        print()
        print(f"  ⏱️  Duration: {self.stats.duration_minutes:.1f} minutes")
        print(f"  📁 Output: {self.output_dir}")
        print("="*70)
        
        if self.stats.real_submissions > 0:
            print(f"\n🎉 SUCCESS! {self.stats.real_submissions} real job applications submitted!")
        else:
            print(f"\n⚠️  No real submissions achieved. Consider:")
