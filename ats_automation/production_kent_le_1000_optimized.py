"""
🚀 KENT LE - 1000 JOB PRODUCTION CAMPAIGN (OPTIMIZED)

Production-grade job application automation
- 50 concurrent BrowserBase sessions (within rate limits)
- Burst protection: 50 requests/minute max
...
class OptimizedCampaignRunner:
    """Production-grade campaign runner with rate limiting"""
    ...
    async def run_campaign(
        ...
        """Run full production campaign with rate limiting"""
        
        self.start_time = datetime.now()
        total_jobs = len(job_urls)
        
        print("\n" + "="*80)
        print("🚀 KENT LE - 1000 JOB PRODUCTION CAMPAIGN (OPTIMIZED)")
        print("="*80)
        ...
        print(f"Rate Limit: 50 requests/minute (BrowserBase)")
        print(f"Started: {self.start_time.isoformat()}")
        print("="*80 + "\n")
        
        # Process in batches to respect rate limits
        batch_size = 50  # BrowserBase burst limit
        all_results = []
        ...
        for batch_num, i in enumerate(range(0, total_jobs, batch_size), 1):
            ...
            print(f"\n{'='*80}")
            print(f"📦 BATCH {batch_num}: Jobs {batch_start+1}-{batch_end}/{total_jobs}")
            print(f"{'='*80}\n")
            
            # Create new router for each batch to clear sessions
            if self.router:
            ...
            if batch_end < total_jobs:
                ...
                print(f"\n⏳ Rate limit: Waiting {wait_time}s before next batch...")
    ...
    async def _process_batch(
        ...
        async def process_single(url: str, index: int):
            nonlocal completed
            async with semaphore:
                try:
                    actual_index = offset + index + 1
                    print(f"[{actual_index}/{total}] Processing: {url[:60]}...")
        ...
        tasks = [process_single(url, i) for i, url in enumerate(urls)]
    ...
    def _generate_report(self, total_jobs: int, location: str) -> Dict:
        ...
        report = {
            "campaign_id": f"kent_le_1000_prod_opt_{self.start_time.strftime('%Y%m%d_%H%M')}",
...
async def main():
    ...
    # Run campaign
    runner = OptimizedCampaignRunner(KENT_LE_PROFILE)
